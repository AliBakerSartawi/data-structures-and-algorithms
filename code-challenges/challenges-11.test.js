...
const validatePin = (pin) => {
  // Solution code here...
  const regex = /^[0-9]{4}$/;
  const noLeadingZeroesPin = (typeof pin === 'string') ? pin.replace('0', '1') : pin;
  const checked = regex.test(parseInt(noLeadingZeroesPin));
...
const validateEmail = (email) => {
  ...
  //// I tested this for an hour on regex101.com
  //// I can't believe i solved it!!!!! regex is the worst
};
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  const regex = /(^(?!\([0-9]{3}\s))(^[(]?)([0-9]{3})([),\s,-]?)([\s]?)([0-9]{3})([\s,-]?)([0-9]{4})$/; 
  // in regex101.com it only worked with /gm at the end
  return regex.test(phoneNumber);
  // I can't believe I constructed this regex from start to finish on my own!!!!!!!!!
  // But I still hate regex... actually, maybe I'm starting to like it
};
