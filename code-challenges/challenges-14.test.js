...
const detectTicTacToeWin = (board) => {
  ...
  function check(row1, row2, row3) {
    // horizontal check
    if (row1[0] === row1[1] && row1[1] === row1[2] && row1[2] !== '') {
      return true;
    }
    if (row2[0] === row2[1] && row2[1] === row2[2] && row2[2] !== '') {
      return true;
    }
    if (row3[0] === row3[1] && row3[1] === row3[2] && row3[2] !== '') {
      return true;
    }
    // vertical check
    if (row1[0] === row2[0] && row2[0] === row3[0] && row3[0] !== '') {
      return true;
    }
    if (row1[1] === row2[1] && row2[1] === row3[1] && row3[1] !== '') {
      return true;
    }
    if (row1[2] === row2[2] && row2[2] === row3[2] && row3[2] !== '') {
      return true;
    }
    // diagonal check
    if (row1[0] === row2[1] && row2[1] === row3[2] && row3[2] !== '') {
      return true;
    }
    if (row1[2] === row2[1] && row2[1] === row3[0] && row3[0] !== '') {
      return true;
    }
    return false;
  }
